...
public class CheckVisitor extends GJDepthFirst<MType, MType> {
	 ...
	 public MType visit(MethodDeclaration n, MType argu) {
		...
		if(m_ret instanceof MIdentifier){	//返回符号表中存在的变量
		...
		}else if(m_ret instanceof MType){	//返回符号表中不存在的值，形如"1","true","new A()","new A().a()"
			if(m_method.owner.classes.FindClassByName(((MType)m_ret).type) != null){	//如果返回的是类，检查父类是否匹配
				MClass r_class = m_method.owner.classes.FindClassByName(((MType)m_ret).type);
				while(r_class!=null && !r_class.parent_name.equals("") && !r_class.parent_name.equals("Object")){
					r_class = m_method.owner.classes.FindClassByName(r_class.parent_name);
					if(r_class!=null && r_class.getName().equals(ret_type)){
						flag = true;
			...
			if(!flag && !((MType)m_ret).type.equals(ret_type))
				error_msg = "METHOD [" + method_name + "] RETURN TYPE [" + ((MType)m_ret).type + "] NOT MATCH, SHOULD RETURN [" + ret_type + "]";
		}else{	//未知错误
...
/*		if(_ret instanceof MIdentifier) {
			if(argu instanceof MClass){
				MClasses m_argu = ((MClass)argu).classes;
			...
			if(argu instanceof MMethod){
				MClasses m_argu = ((MMethod)argu).owner.classes;
				if( ((MClasses)m_argu).getIndex(_ret.getName()) == -1 )
...
*/		return _ret;
						...
						for(int i = 0; i < m_method.par_list.size(); i ++){
							if(m_args.args_list.elementAt(i) instanceof MIdentifier){
								...
								if(((MMethod)argu).getVar(m_args.args_list.elementAt(i).getName())!=null &&
									...
									MClass s_class = ((MMethod)argu).owner.classes.FindClassByName(((MMethod)argu).getVar(m_args.mj_args.elementAt(i).getName()).type);
									while(s_class!=null && !(s_class.father).equals("") && !(s_class.father).equals("Object")){
										s_class = ((MMethod)argu).owner.classes.FindClassByName(s_class.father);
								...
								if(((MTypename)m_args.mj_args.elementAt(i)).type != 
									...
									boolean flg = false;
									MClass s_class = ((MMethod)argu).owner.classes.FindClassByName(((MTypename)m_args.mj_args.elementAt(i)).type);
									while(s_class!=null && !(s_class.father).equals("") && !(s_class.father).equals("Object")){
										s_class = ((MMethod)argu).owner.classes.FindClassByName(s_class.father);
										if(s_class.getName().equals(m_method.mj_para.elementAt(i).type)){
											flg = true;
	 ...
	 public MType visit(PrimaryExpression n, MType argu) {
			...
			if(tmp instanceof MIdentifier){
				...
				if( ((MMethod)argu).getVar(var_name) == null ){
					...
					if( ((MMethod)argu).FindVar(tmp) != -1 && ((MMethod)argu).getVar(var_name).isInited == false ){
	 ...
	 public MType visit(AllocationExpression n, MType argu) {
			...
			if( (((MMethod)argu).owner.classes).FindClass(m_class) == -1){ //UNFINISHED
