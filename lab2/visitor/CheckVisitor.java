...
public class CheckVisitor extends GJDepthFirst<MType, MType> {
    ...
    public MType visit(MainClass n, MType argu) {
        MType _ret = null;
        n.f0.accept(this, argu);
        String class_name = ((MIdentifier) n.f1.accept(this, argu)).getName(); //从Identifier()获得主类名
        MClass m_class = ((MClassList) argu).FindClassByName(class_name);
        MMethod m_method = m_class.method_list.elementAt(0); //主类的main method 一定在0位置
        n.f2.accept(this, argu);
    ...
    public MType visit(ClassExtendsDeclaration n, MType argu) {
        ...
        if (((MClassList) argu).FindClassByName(father_class_name) == null) {
            ((MClassList) argu).FindClassByName(class_name).parent_name = ""; //没有父类则记为""
        ...
        MClass m_class = ((MClassList) argu).FindClassByName(class_name);
        MClass c = m_class;
        while (c != null && !c.parent_name.equals("")) { //java不能用==
        ...
        n.f4.accept(this, argu);
        n.f5.accept(this, m_class); //VarDeclaration()* 类的成员变量
        n.f6.accept(this, m_class); //MethodDeclaration()* 类的方法
        n.f7.accept(this, argu);
        return _ret;
    }
    ...
    /**
     * 方法定义错误：
     * 与父类同名方法不匹配（返回类型/参数列表）
     * 返回的Expression()与Type()不匹配
     */
    public MType visit(MethodDeclaration n, MType argu) {
        ...
        boolean flag = false;
        if (m_ret instanceof MIdentifier) {    //Expression()是已知类和变量名
            ...
            if (ret_var != null && m_method.owner.classes.FindClassByName(ret_var.type) != null) {
                ...
                while (r_class != null && !r_class.parent_name.equals("")) { //是类则检查有无继承关系
        ...
        } else if (m_ret instanceof MTypename) {    //Expression()是还没算的表达式
    ...
    public MType visit(AssignmentStatement n, MType argu) {
        ...
        if (((MMethod) argu).getVar(m_id.getName()) == null) { //左值不存在
...
//            System.out.println(m_var.owner.name);
            if (t instanceof MIdentifier) { //右值在符号表
...
//                System.out.println(m_var2.owner.name);
            } else if (t instanceof MTypename) { //右值不在符号表，推算类型
...
//                System.out.println("???");
    ...
    public MType visit(ArrayAssignmentStatement n, MType argu) {
        ...
        else if (!((MMethod) argu).getVar(m_id.getName()).type.equals("Array")) //左值不是数组
        ...
        // 其次检查下标是否为Int
        MType p = n.f2.accept(this, argu); //Expression() 数组下标
        if (p instanceof MIdentifier) {
            MVar v = ((MMethod) argu).getVar(p.getName());
            if (v != null && !v.type.equals("Int")) // 下标不是int
        ...
        MType t = n.f5.accept(this, argu); //Expression() 右值
    ...
    /**
     * 打印语句错误：
     * f2 Expression() 不是int(minijava要求）
     */
    ...
    public MType visit(MessageSend n, MType argu) {
        ...
        if (t instanceof MTypename) {
            ...
            MVar m_var = ((MMethod) argu).getVar(t.getName()); //基本类型不能调用方法
        ...
        if (m_class != null) {
            ...
            if (m_method != null) {
                ...
                if (m_method.par_list.size() != arg_size) { //参数输两
                        ...
                        if (m_args.args_list.elementAt(i) instanceof MIdentifier) {
                            ...
                            if (((MMethod) argu).getVar(m_args.args_list.elementAt(i).getName()) != null &&
                                    !((MMethod) argu).getVar(m_args.args_list.elementAt(i).getName()).type.equals(
                                            m_method.par_list.elementAt(i).type)) { //每个参数的类型
    ...
    public MType visit(ExpressionRest n, MType argu) {
        ...
        MType arg = (MType) n.f1.accept(this, ((MArgs) argu).argu); //ExpRest 传回祖父！
    ...
    public MType visit(BracketExpression n, MType argu) {
        MType _ret = null;
        n.f0.accept(this, argu);
        _ret = n.f1.accept(this, argu); //不管括号
        n.f2.accept(this, argu);
        return _ret;
    }
